...
export class Storage {
  ...
  constructor(config: StorageConfig = {}) {
    const dbPath = config.dbPath || path.join(process.cwd(), 'aios-runtime.db');
  ...
  getWorkspace(id: string): Workspace {
    ...
    const tabs = tabsStmt.all(id) as Array<{ tabId: string; providerId: string; agentMode: string }>;
  ...
  getPermission(workspaceId: string, tool: string): PermissionState {
    ...
    const result = stmt.get(workspaceId, tool) as { state: string } | undefined;
    return result?.state as PermissionState || 'ask';
  }
  ...
  getSession(id: string): ReasoningSession {
    ...
    return {
      id: session.id,
      role: session.role as 'planner' | 'worker',
      tabId: session.tabId,
